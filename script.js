...
video.addEventListener("play", () => {
  ...
  setInterval(async () => {
    const detetctions = await faceapi
      .detectAllFaces(video, new faceapi.TinyFaceDetectorOptions())
      .withFaceLandmarks()
      .withFaceExpressions()
      .withAgeAndGender();
    if (detetctions.length>0) {
      ...
      //can use below 2 lines of code to get age median
      ...
      new faceapi.draw.DrawTextField(
        [
          `${resizeDetections[0].gender} ${faceapi.utils.round(
            resizeDetections[0].age,
            0
          )} years`,
        ],
        bottomRight
